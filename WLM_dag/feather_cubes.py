...
def main():
    ...
    vla_cube = SpectralCube.read(interf_cubename, use_dask = True)
    vla_cube.allow_huge_operations = True
    gbt_cube = SpectralCube.read(sd_cubename, use_dask = True)
    ...
    #with fits.open(gbt_cube, mode="update") as hdulist:
    ...
    # Append the sdfactor used into the header.
    with fits.open(this_feathered_filename, mode="update") as hdulist:
        hdulist[0].header["COMMENT"] = f"Feathered with uvcombine using sdfactor={sdfactor}"
        hdulist.flush()
