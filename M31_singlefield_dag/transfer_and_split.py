"""
04/16/2024
Split out a user provided spectral window and field name from input measurement set
User inputs:
...
-t --time_bin - <required> time over which to average visibilities in units of sec (e.g., 5)
-f --field_id - <required> name of field to split out
...
parser.add_argument('-f', '--field_id', help='<required> name of field to split out', required = True)
...
def main():
	...
	output_vis = '%s_spw.%s' % (ms_path, field_id)
